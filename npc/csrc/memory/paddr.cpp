...
extern "C" void pmem_read(long long raddr, long long *rdata) {
  ...
  if(raddr>=0x80000000){
	...
	if(raddr >= CONFIG_MTRACE_START && raddr <= CONFIG_MTRACE_END){
	  	printf("\33[1;32m""mtrace: read    addr:0x%llx(0x%llx)   data:0x%llx \n""\33[0m",\
...
extern "C" void pmem_write(long long waddr, long long wdata, char wmask) {
  ...
  if(waddr>=0x80000000){
	...
	if(waddr >= CONFIG_MTRACE_START && waddr <= CONFIG_MTRACE_END){
	  	printf("\33[1;31m""mtrace: write   addr:0x%llx(0x%llx)   data:0x%llx   wmask:%x     at pc=%lx    inst num=%ld\n""\33[0m",\
...
//  *(uint32_t*)&pmem[0x00000004]=0x00100133; //add sp, zero, ra
//  *(uint32_t*)&pmem[0x00000008]=0x00518193; //gp = gp + 5
//  *(uint32_t*)&pmem[0x0000000c]=0x00720213; //tp = tp + 7
//					    //
//  *(uint32_t*)&pmem[0x00000010]=0x00600593; //a1 = $0 + 6
