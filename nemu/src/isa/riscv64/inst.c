...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, word_t shamt=imm & 0x000000000000003f;word_t src1_s=src1>>shamt;word_t mask_t = 0xffffffffffffffff;mask_t=mask_t<<(64-shamt);R(dest)=(BITS(src1,63,63)==1)?(mask_t|src1_s):src1_s);  
