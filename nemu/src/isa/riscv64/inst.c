...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, src1=0xf000000000000000;word_t src1_s=src1>>BITS(imm,5,0);R(dest)=(BITS(src1,63,63)==1)?((BITS(0xffffffff,BITS(imm,5,0)-1,0)<<(64-BITS(imm,5,0)))|src1_s):0x300  ); //;else R(dest)=src1_s);
		  //word_t shamt=BITS(imm,5,0);int64_t a=64-shamt;R(dest)=SEXT(  BITS(  (src1>> shamt), (64-shamt), 0),a    )    );//     !!!!!!!!TO DO 
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + imm, 1)|0x00000000);  //!!!!!bitnum
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = ((sword_t)src1>=(sword_t)src2)?(s->pc+imm):s->dnpc);     //!!!!!todo
