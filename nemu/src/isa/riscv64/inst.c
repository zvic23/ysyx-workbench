...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, word_t src1_s=src1>>BITS(imm,5,0);word_t shamt=BITS(imm,5,0);R(dest)=(BITS(src1,63,63)==1)?((BITS(0xffffffff,shamt-1,0)<<(64-shamt))|src1_s):src1_s  );              //     !!!!!!!!have doubt 
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, src1=0x1232441291111111;src2=0x8000000080000001;	word_t src1_32=BITS(src1,31,0);word_t src2_32=BITS(src2,31,0);word_t quotient=0;word_t src1_32_cpl=(src1_32^0xffffffff)+1; word_t src2_32_cpl=(src2_32^0xffffffff)+1; 
