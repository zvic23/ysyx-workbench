...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = (src1<imm)?1:0);//if(src1<imm)R(dest)=0x1;else R(dest)=0);        !!!!!!!!!!sign compare
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (sword_t)src1 >> BITS(imm, 5, 0));//src1 >> BITS(imm, 5, 0));
