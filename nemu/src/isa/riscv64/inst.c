...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, word_t shamt=BITS(imm,5,0);word_t src1_s=src1>>shamt;R(dest)=(BITS(src1,63,63)==1)?((BITS(0xffffffffffffffff,shamt-1,0)<<(64-shamt))|src1_s):src1_s);              //     !!!!!!!!have doubt 
  ...
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, word_t match=0;//src1=0x0000000000000000;src2=0x8000000000000001;	
	if(BITS(src1,63,63)==BITS(src2,63,63))match=(src1>=src2)?1:0;
	if(BITS(src1,63,63)!=BITS(src2,63,63))match=(BITS(src1,63,63)==0)?1:0;
	s->dnpc = (match)?(s->pc+imm):s->dnpc);     //!!!!!have doubt and to be optimized                                   zsl:there is something wrong in bge and bgeu
  
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(dest) = src1 % src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divu   , R,	R(dest) = src1 / src2); 
